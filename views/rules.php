<main role="main">
	<div class="content" role="content">
		<h2 role="heading">Reglur</h2>
		
			<p>Leikmaðurinn byrjar á að draga 4 spil og geyma á hendi sér.</p>
			<p>Markmið leiksins er að losa sig við spil og sigur næst með því að vera með 2 spil eða færri á hendi.</p>
			<p>Þegar stokkurinn klárast er hægt að færa seinasta spilið í hendinni fremst</p>
			<p>Hægt er að losa sig við spil á tvennan hátt:</p>
			<ul>
				<li>Ef spil með 2 spilum á milli sín hafa sömu sort er hægt að henda spilunum á milli.</li>
				<li>Ef spil með 2 spilum á milli sín hafa sama gildið er hægt að henda öllum 4 spilunum.</li>
			</ul>			
			<br>
			<br>
			<h3 role="heading">Dæmi til að losa sig við 2 spil:</h3>
			<div>
				<img class='img' id='examplecard' src='/pandakapall/pandakapall/img/H3.png' alt="Card: H3" role="img">
				<img class='img' id='examplecard' src='/pandakapall/pandakapall/img/S7.png' alt="Card: S7" role="img">
				<img class='img' id='examplecard' src='/pandakapall/pandakapall/img/C5.png' alt="Card: C5" role="img">
				<img class='img' id='examplecard' src='/pandakapall/pandakapall/img/H10.png' alt="Card: H10" role="img">
			</div>

			<p>Hér eru spilin sem hægt er að losa sig við:</p>
			<div>
				<img class='img' id='examplecard' src='/pandakapall/pandakapall/img/S7.png' alt="Card: S7" role="img">
				<img class='img' id='examplecard' src='/pandakapall/pandakapall/img/C5.png' alt="Card: C5" role="img">
			</div>

			<p>Þar sem eftirfarandi spil eru af sömu sort.</p>
			<div>
				<img class='img' id='examplecard' src='/pandakapall/pandakapall/img/H3.png' alt="Card: H3" role="img">
				<img class='img' id='examplecard' src='/pandakapall/pandakapall/img/H10.png' alt="Card: H10" role="img">
			</div>

			<p>Alltaf skal velja fyrsta spil sem á að fjarlægja, þ.e. þetta spil:</p>
			<div>
				<img class='img' id='examplecard' src='/pandakapall/pandakapall/img/S7.png' alt="Card: S7" role="img">
			</div>
			<br>
			<br>
			<h3 role="heading">Dæmi til að losa sig við 4 spil:</h3>
			<div>
				<img class='img' id='examplecard' src='/pandakapall/pandakapall/img/H3.png' alt="Card: H3" role="img">
				<img class='img' id='examplecard' src='/pandakapall/pandakapall/img/S7.png' alt="Card: S7" role="img">
				<img class='img' id='examplecard' src='/pandakapall/pandakapall/img/C5.png' alt="Card: C5" role="img">
				<img class='img' id='examplecard' src='/pandakapall/pandakapall/img/D3.png' alt="Card: D3" role="img">
			</div>

			<p>Hér er hægt að losa sig við öll 4 spilin því eftirfarandi spil eru með sama gildið:</p>
			<div>
				<img class='img' id='examplecard' src='/pandakapall/pandakapall/img/H3.png' alt="Card: H3" role="img">
				<img class='img' id='examplecard' src='/pandakapall/pandakapall/img/D3.png' alt="Card: D3" role="img">
			</div>

			<p>Alltaf skal velja fyrsta spil sem á að fjarlægja, semsagt:</p>
			<div>
				<img class='img' id='examplecard' src='/pandakapall/pandakapall/img/H3.png' alt="Card: H3" role="img">
			</div>
			<p>Eins og staðan er núna velur leikurinn að fjarlægja 4 spil fram yfir 2 ef hægt er að gera bæði</p>
			<br>
			<br>
			<h3>Stigagjöf</h3>
			<p>Gefin eru 100 stig til að byrja með.</p>
			<p>Hint dregur engin stig en Put Last First og Draw draga 1 stig hvort.</p>
			<p>Veitt eru 10 stig fyrir að fjarlægja 2 spil en 25 stig fyrir að fjarlægja 4 spil.</p>
			<p>Undo virkar á eftirfarandi hátt:</p>
			<p>	Hækkar upp refsingu um 2 stig hvert sinn sem klikkað er á undo,
				ef ýtt er oft á undo í röð er þó dregið aðeins 1 stig fyrir hvert undo á eftir því fyrsta
				EN refsingin heldur áfram að hækka. Einungis fyrsta undo eftir aðra aðgerð dregur niður 
				um fulla refsingu.
			</p>
	</div>
</main>